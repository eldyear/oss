...
function updateTable(language, data) {
    ...
    flipRowsSequentially(language); // Запускаем flip-анимацию
}

// Анимация переворота строк таблицы
function flipRowsSequentially(language) {
    const tbody = document.getElementById(`tbody-${language}`);
    ...
    rows.forEach((row, index) => {
        setTimeout(() => {
            ...
            setTimeout(() => row.classList.remove('flip'), 600); // Сбрасываем flip через 0.6 секунды
        }, index * 100); // Задержка в 0.1 секунды между строками
    });
}
